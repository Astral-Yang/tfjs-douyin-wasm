...
(function(global, factory) {
...
})(TTWebAssembly, (function(exports, tfjsCore) {
	'use strict';
	...
	***************************************************************************** */
	...
	function fusedBatchMatMul(args) {
		...
		var batchDims = tfjsCore.broadcast_util.assertAndGetBroadcastShape(a.shape.slice(0, -2), b.shape.slice(
			0, -2));
		var out = backend.makeOutput(__spreadArray(__spreadArray([], __read(batchDims), false), [leftDim,
			rightDim
		], false), a.dtype);
		...
		wasmFusedMatMul(aId, aShapeBytes, a.shape.length, bId, bShapeBytes, b.shape.length, transposeA,
			transposeB, fusedActivation, biasId, preluActivationWeightsId, leakyreluAlpha || 0, outId);
		return out;
	}
	...
	function createBinaryKernelConfig(kernelName, supportsFullBroadcast, dtype) {
		...
		function kernelFunc(args) {
			...
			var kernelFunc = function() {
				return wasmFunc(aId, aShapeBytes, a.shape.length, bId, bShapeBytes, b.shape.length,
	...
	function avgPool(args) {
		...
		var convInfo = tfjsCore.backend_util.computePool2DInfo(x.shape, filterSize, strides, 1 /* dilations */ ,
			pad, dimRoundingMode);
		...
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmAvgPool(xId, x.shape[0], x.shape[1], x.shape[2], filterHeight, filterWidth, padTop, padRight,
			padBottom, padLeft, strideHeight, strideWidth, channels, outId);
		return out;
	}
	var avgPoolConfig = {
	...
	function avgPool3D(args) {
		...
		var convInfo = tfjsCore.backend_util.computePool3DInfo(x.shape, filterSize, strides,
			/*dilations=*/
			1, pad, dimRoundingMode, dataFormat);
		var out = backend.makeOutput(convInfo.outShape, x.dtype);
		wasmAvgPool3D(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(out.dataId).id, convInfo
			.batchSize,
			// Since Pool3D ops (AvgPool3D and MaxPool3D) support 3D filter only, in
			// channels should always equal to out channels.
			/*channelSize=*/
			convInfo.inChannels, convInfo.inDepth, convInfo.inHeight, convInfo.inWidth, convInfo.outDepth,
			convInfo.outHeight, convInfo.outWidth, convInfo.strideDepth, convInfo.strideHeight, convInfo
			.strideWidth, convInfo.dilationDepth, convInfo.dilationHeight, convInfo.dilationWidth, convInfo
			.effectiveFilterDepth, convInfo.effectiveFilterHeight, convInfo.effectiveFilterWidth, convInfo
	...
	function avgPool3DGrad(args) {
		...
		var filterSize = attrs.filterSize,
			...
			pad = attrs.pad,
			dimRoundingMode = attrs.dimRoundingMode;
		var convInfo = tfjsCore.backend_util.computePool3DInfo(input.shape, filterSize, strides, /*dilations=*/
			1, pad, dimRoundingMode);
		var dx = backend.makeOutput(input.shape, input.dtype);
		wasmAvgPool3DGrad(backend.dataIdMap.get(dy.dataId).id, backend.dataIdMap.get(dx.dataId).id, convInfo
			.batchSize,
			// Since Pool3D ops (AvgPool3D and MaxPool3D) support 3D filter only, in
			// channels should always equal to out channels.
			/*channelSize=*/
			...
			.strideWidth, convInfo.dilationDepth, convInfo.dilationHeight, convInfo.dilationWidth, convInfo
			.effectiveFilterDepth, convInfo.effectiveFilterHeight, convInfo.effectiveFilterWidth, convInfo
			.padInfo.front, convInfo.padInfo.top, convInfo.padInfo.left, convInfo.filterDepth, convInfo
			.filterHeight, convInfo.filterWidth);
		return dx;
	}
	...
	function avgPoolGrad(args) {
		...
		var convInfo = tfjsCore.backend_util.computePool2DInfo(input.shape, filterSize, strides,
			...
			1, pad);
		var dx = backend.makeOutput(input.shape, input.dtype);
		wasmAvgPoolGrad(backend.dataIdMap.get(dy.dataId).id, backend.dataIdMap.get(dx.dataId).id, convInfo
			.batchSize,
			// Since Pool ops (AvgPool and MaxPool) support 2D filter only, in
			// channels should always equal to out channels.
			/*channelSize=*/
			convInfo.inChannels, convInfo.inHeight, convInfo.inWidth, convInfo.outHeight, convInfo.outWidth,
			convInfo.strideHeight, convInfo.strideWidth, convInfo.dilationHeight, convInfo.dilationWidth,
			convInfo.effectiveFilterHeight, convInfo.effectiveFilterWidth, convInfo.padInfo.top, convInfo
			.padInfo.left, convInfo.filterHeight, convInfo.filterWidth);
		return dx;
	}
	...
	function reshape(args) {
		...
		tfjsCore.util.assert(xSize === tfjsCore.util.sizeFromShape($shape), function() {
			return "new shape: ".concat($shape, ", old shape: ").concat(x.shape,
					". New shape and old ") +
	...
	function batchMatMul(args) {
		...
		var outShapeOuterDims = tfjsCore.broadcast_util.assertAndGetBroadcastShape(a.shape.slice(0, -2), b.shape
			.slice(0, -2));
		var outShape = outShapeOuterDims.concat([outerShapeA, outerShapeB]);
		tfjsCore.util.assert(innerShapeA === innerShapeB, function() {
			return "Error in matMul: inner shapes (".concat(innerShapeA, ") and (") +
		...
		var a3dShape = transposeA ? [batchDimA, innerShapeA, outerShapeA] : [batchDimA, outerShapeA,
			innerShapeA
		];
		var b3dShape = transposeB ? [batchDimB, outerShapeB, innerShapeB] : [batchDimB, innerShapeB,
			outerShapeB
		];
		...
		wasmBatchMatMul(a3dId, aShapeBytes, a3d.shape.length, b3dId, bShapeBytes, b3d.shape.length, transposeA,
			transposeB, outId);
	...
	function sliceImpl(vals, begin, size, shape, dtype) {
		...
		for (var i = 0; i < outBuf.size; ++i) {
			...
			outBuf.set.apply(outBuf, __spreadArray([inBuf.get.apply(inBuf, __spreadArray([], __read(inLoc),
				false))], __read(outLoc), false));
		}
		if (dtype === 'string') {
	...
	var StringNGramsOp = /** @class */ (function() {
		function StringNGramsOp(separator, nGramWidths, leftPad, rightPad, padWidth,
		...
		StringNGramsOp.prototype.createNGrams = function(data, splitIndex, output, outputStartIndex,
			numNGrams, nGramWidth) {
		...
		StringNGramsOp.prototype.compute = function(data, splits) {
			...
			if (splitsSize > 0) {
				...
				for (var i = 1; i < splitsSize; ++i) {
					...
					if (!validSplits) {
						throw new Error("Invalid split value ".concat(splits[i], ", must be in [")
							.concat(prevSplit, ", ").concat(inputDataSize, "]"));
					}
					prevSplit = splits[i];
				}
				if (prevSplit !== inputDataSize) {
					throw new Error("Last split value must be data size. Expected ".concat(
						inputDataSize, ", got ").concat(prevSplit));
				}
			}
			...
			var _loop_3 = function(i) {
				...
				this_3.nGramWidths.forEach(function(nGramWidth) {
					...
					_this.createNGrams(data, splitIndex, nGrams, outputStartIdx,
						numNGrams, nGramWidth);
				...
				if (this_3.preserveShort && outputStartIdx === nGramsSplits[i]) {
					...
					var numNGrams = 1;
					this_3.createNGrams(data, splitIndex, nGrams, outputStartIdx, numNGrams,
						nGramWidth);
				}
			};
			var this_3 = this;
	...
	function stringNGramsImpl(data, dataSplits, separator, nGramWidths, leftPad, rightPad, padWidth,
		preserveShortSequences) {
	...
	function broadcastArgs(args) {
		...
		var broadcastShape = tfjsCore.backend_util.assertAndGetBroadcastShape(Array.from(s0Vals), Array.from(
			s1Vals));
	...
	function concat(args) {
		...
		if ($inputs[0].dtype === 'string') {
			...
			outShape =
				tfjsCore.backend_util.computeOutShape(inputs2D.map(function(t) {
					return t.shape;
				}), 1 /* axis */ );
	...
	function conv2d(args) {
		...
		var convInfo = tfjsCore.backend_util.computeConv2DInfo(x.shape, filter.shape, strides, dilations, pad,
		...
		var out = backend.makeOutput(convInfo.outShape, 'float32');
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmConv2d(xId, x.shape[0], x.shape[1], x.shape[2], filterId, filterHeight, filterWidth, padTop,
			padRight, padBottom, padLeft, isSamePad, dilationHeight, dilationWidth, strideHeight,
			strideWidth, inputChannels, outputChannels, outId);
		return out;
	}
	var conv2DConfig = {
	...
	function conv2DBackpropInput(args) {
		...
		var dilations = 1;
		var $dataFormat = tfjsCore.backend_util.convertConv2DDataFormat(dataFormat);
		var convInfo = tfjsCore.backend_util.computeConv2DInfo(inputShape, filter.shape, strides, dilations,
		...
		var filterId = backend.dataIdMap.get(filter.dataId).id;
		wasmConv2DBackpropInput(dyId, filterId, batchSize, filterHeight, filterWidth, inHeight, inWidth,
			inChannels, outHeight, outWidth, outChannels, strideHeight, strideWidth, topPad, leftPad, fltS0,
			fltS1, fltS2, xBatchStride, xRowStride, xColStride, xChannelStride, yBatchStride, yRowStride,
			yColStride, yChannelStride, outId);
		return out;
	}
	...
	function conv3D(args) {
		...
		var out = backend.makeOutput(convInfo.outShape, x.dtype);
		wasmConv3D(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(filter.dataId).id, backend
			.dataIdMap.get(out.dataId).id, convInfo.batchSize, convInfo.inDepth, convInfo.inHeight, convInfo
			.inWidth, convInfo.inChannels, convInfo.outDepth, convInfo.outHeight, convInfo.outWidth,
			convInfo.outChannels, convInfo.strideDepth, convInfo.strideHeight, convInfo.strideWidth,
			convInfo.dilationDepth, convInfo.dilationHeight, convInfo.dilationWidth, convInfo.filterDepth,
			convInfo.filterHeight, convInfo.filterWidth, convInfo.padInfo.front, convInfo.padInfo.top,
	...
	function conv3DBackpropFilterV2(args) {
		...
		wasmConv3DBackpropFilterV2(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(dy.dataId).id,
			backend.dataIdMap.get(dw.dataId).id, convInfo.batchSize, convInfo.inDepth, convInfo.inHeight,
			convInfo.inWidth, convInfo.inChannels, convInfo.outDepth, convInfo.outHeight, convInfo.outWidth,
			convInfo.outChannels, convInfo.strideDepth, convInfo.strideHeight, convInfo.strideWidth,
			convInfo.dilationDepth, convInfo.dilationHeight, convInfo.dilationWidth, convInfo.filterDepth,
			convInfo.filterHeight, convInfo.filterWidth, convInfo.padInfo.front, convInfo.padInfo.top,
			convInfo.padInfo.left);
	...
	function conv3DBackpropInputV2(args) {
		...
		var convInfo = tfjsCore.backend_util.computeConv3DInfo(inputShape, filter.shape, strides, /*dilations=*/
			1, pad);
		var dx = backend.makeOutput(convInfo.inShape, dy.dtype);
		wasmConv3DBackpropInputV2(backend.dataIdMap.get(filter.dataId).id, backend.dataIdMap.get(dy.dataId).id,
			backend.dataIdMap.get(dx.dataId).id, convInfo.batchSize, convInfo.inDepth, convInfo.inHeight,
			convInfo.inWidth, convInfo.inChannels, convInfo.outDepth, convInfo.outHeight, convInfo.outWidth,
			convInfo.outChannels, convInfo.strideDepth, convInfo.strideHeight, convInfo.strideWidth,
			convInfo.dilationDepth, convInfo.dilationHeight, convInfo.dilationWidth, convInfo.filterDepth,
			convInfo.filterHeight, convInfo.filterWidth, convInfo.padInfo.front, convInfo.padInfo.top,
	...
	function cropAndResize(args) {
		...
		wasmCropAndResize(imagesId, boxesId, boxIndId, numBoxes, imagesShapeBytes, cropHeight, cropWidth,
			InterpolationMethod[method], extrapolationValue, outId);
		if (castedData != null) {
	...
	function cumprod(args) {
		...
		wasmCumprod(permutedXId, exclusive ? 1 : 0, reverse ? 1 : 0, finalDim, permutedOutId, CppDType[x
			.dtype]);
	...
	function denseBincount(args) {
		...
		wasmDenseBincount(tensorId(x), new Uint8Array(new Int32Array(x.shape).buffer), x.shape.length, size,
			hasWeights, tensorId(weights), CppDType[weights.dtype], binaryOutput, tensorId(out));
		return out;
	}
	...
	function depthToSpace(args) {
		...
		var outputShape = (dataFormat === 'NHWC') ? [batchSize, outputHeight, outputWidth, outputDepth] : [
		...
		wasmDepthToSpace(xId, blockSize, channelsLast, xStridesBytes, x.shape.length - 1, outputShapeBytes,
			outStridesBytes, outputShape.length, outId);
		return out;
	}
	...
	function depthwiseConv2d(args) {
		...
		var convInfo = tfjsCore.backend_util.computeConv2DInfo(x.shape, filter.shape, strides, $dilations, pad,
			dimRoundingMode, true /* depthwise */ );
		var filterHeight = convInfo.filterHeight;
		var filterWidth = convInfo.filterWidth;
		var padTop = convInfo.padInfo.top;
		var padRight = convInfo.padInfo.right;
		...
		var out = backend.makeOutput(convInfo.outShape, 'float32');
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmDepthwiseConv2d(xId, x.shape[0], x.shape[1], x.shape[2], filterId, filterHeight, filterWidth,
			padTop, padRight, padBottom, padLeft, isSamePad, dilationHeight, dilationWidth, strideHeight,
			strideWidth, inputChannels, outputChannels, outId);
		return out;
	}
	...
	function diag(args) {
		...
		var x = inputs.x;
		var xSize = tfjsCore.util.sizeFromShape(x.shape);
		var out = backend.makeOutput(__spreadArray(__spreadArray([], __read(x.shape), false), __read(x.shape),
			false), x.dtype);
		wasmDiag(backend.dataIdMap.get(x.dataId).id, CppDType[x.dtype], xSize, backend.dataIdMap.get(out.dataId)
			.id);
		return out;
	}
	var diagConfig = {
	...
	function dilation2D(args) {
		...
		if (x.dtype !== filter.dtype) {
			throw new Error("Dilation2D error: x must have the same dtype as filter. Got ".concat(x.dtype,
				" and ").concat(filter.dtype));
		}
		...
		wasmDilation2D(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(filter.dataId).id, backend
			.dataIdMap.get(out.dataId).id, CppDType[x.dtype], dilationInfo.batchSize,
			/*depth=*/
			dilationInfo.inChannels, dilationInfo.inHeight, dilationInfo.inWidth, dilationInfo.outHeight,
			dilationInfo.outWidth, dilationInfo.strideHeight, dilationInfo.strideWidth, dilationInfo
			.dilationHeight, dilationInfo.dilationWidth, dilationInfo.filterHeight, dilationInfo
			.filterWidth, dilationInfo.padInfo.top, dilationInfo.padInfo.left);
		return out;
	}
	...
	function dilation2DBackpropFilter(args) {
		...
		if (x.dtype !== filter.dtype || x.dtype !== dy.dtype) {
			throw new Error("Dilation2DBackpropFilter error: x must have the same dtype as filter and dy. Got "
				.concat(x.dtype, ", ").concat(filter.dtype, ", and ").concat(dy.dtype));
		}
		var dilationInfo = tfjsCore.backend_util.computeDilation2DInfo(x.shape, filter.shape, strides, pad,
			/*dataFormat=*/
			'NHWC', dilations);
		var gradients = backend.makeOutput(filter.shape, filter.dtype);
		wasmDilation2DBackpropFilter(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(filter.dataId)
			.id, backend.dataIdMap.get(dy.dataId).id, backend.dataIdMap.get(gradients.dataId).id, CppDType[x
				.dtype], dilationInfo.batchSize,
			/*depth=*/
			dilationInfo.inChannels, dilationInfo.inHeight, dilationInfo.inWidth, dilationInfo.outHeight,
			dilationInfo.outWidth, dilationInfo.strideHeight, dilationInfo.strideWidth, dilationInfo
			.dilationHeight, dilationInfo.dilationWidth, dilationInfo.filterHeight, dilationInfo
	...
	function dilation2DBackpropInput(args) {
		...
		var strides = attrs.strides,
			...
			dilations = attrs.dilations;
		if (x.dtype !== filter.dtype || x.dtype !== dy.dtype) {
			throw new Error("Dilation2DBackpropInput error: x must have the same dtype as filter and dy. Got "
				.concat(x.dtype, ", ").concat(filter.dtype, ", and ").concat(dy.dtype));
		}
		var dilationInfo = tfjsCore.backend_util.computeDilation2DInfo(x.shape, filter.shape, strides, pad,
			/*dataFormat=*/
		...
		wasmDilation2DBackpropInput(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(filter.dataId).id,
			backend.dataIdMap.get(dy.dataId).id, backend.dataIdMap.get(gradients.dataId).id, CppDType[x
				.dtype], dilationInfo.batchSize,
			/*depth=*/
			dilationInfo.inChannels, dilationInfo.inHeight, dilationInfo.inWidth, dilationInfo.outHeight,
			dilationInfo.outWidth, dilationInfo.strideHeight, dilationInfo.strideWidth, dilationInfo
	...
	function setup$K(backend) {
		wasmBatchNorm = backend.wasm.cwrap(tfjsCore.FusedBatchNorm, null /* void */ , ['number', 'number',
	...
	function fusedConv2d(args) {
		...
		var preluActivationWeightsId = preluActivationWeights == null ?
			0 :
			backend.dataIdMap.get(preluActivationWeights.dataId).id;
		wasmFusedConv2d(xId, batchSize, inHeight, inWidth, filterId, filterHeight, filterWidth, biasId, padTop,
			padRight, padBottom, padLeft, isSamePad, dilationHeight, dilationWidth, strideHeight,
			strideWidth, inputChannels, outputChannels, fusedActivation, preluActivationWeightsId,
	...
	function fusedDepthwiseConv2d(args) {
		...
		var preluActivationWeightsId = preluActivationWeights == null ?
			0 :
			backend.dataIdMap.get(preluActivationWeights.dataId).id;
		wasmFusedDepthwiseConv2d(xId, batchSize, inHeight, inWidth, filterId, filterHeight, filterWidth, biasId,
			padTop, padRight, padBottom, padLeft, isSamePad, dilationHeight, dilationWidth, strideHeight,
			strideWidth, inputChannels, outputChannels, fusedActivation, preluActivationWeightsId,
			leakyreluAlpha || 0, outId);
		return out;
	...
	function gatherNd(args) {
		...
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmGatherNd(xId, CppDType[params.dtype], indicesId, numSlices, sliceRank, sliceSize, stridesBytes,
			outId);
		return out;
	}
	var gatherNdConfig = {
	...
	function gatherV2(args) {
		...
		var _loop_1 = function(i) {
			...
			tfjsCore.util.assert(index <= axisDim - 1 && index >= 0, function() {
				return "GatherV2: the index value ".concat(index, " is not in [0, ").concat(
					axisDim - 1, "]");
			});
		};
		...
		var outStridesBytes = new Uint8Array(new Int32Array(tfjsCore.util.computeStrides(flattenOutputShape))
			.buffer);
		wasmGather(xId, CppDType[x.dtype], xStridesBytes, stridesSize, indicesId, shapeInfo.batchSize,
	...
	function lrn(args) {
		...
		wasmLRN(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(out.dataId).id,
			/*channels=*/
			x.shape[3], depthRadius, bias, alpha, beta);
		return out;
	}
	var lrnConfig = {
	...
	function lrnGrad(args) {
		...
		wasmLRNGrad(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(y.dataId).id, backend.dataIdMap
			.get(dy.dataId).id, backend.dataIdMap.get(dx.dataId).id,
			/*channels=*/
			dy.shape[3], depthRadius, bias, alpha, beta);
		return dx;
	}
	var lrnGradConfig = {
	...
	function maxPool(args) {
		...
		var out = backend.makeOutput(convInfo.outShape, 'float32');
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmMaxPool(xId, x.shape[0], x.shape[1], x.shape[2], filterHeight, filterWidth, padTop, padRight,
			padBottom, padLeft, dilationHeight, dilationWidth, strideHeight, strideWidth, inputChannels,
	...
	function maxPool3D(args) {
		...
		var convInfo = tfjsCore.backend_util.computePool3DInfo(x.shape, filterSize, strides,
			...
			1, pad, dimRoundingMode, dataFormat);
		var out = backend.makeOutput(convInfo.outShape, x.dtype);
		wasmMaxPool3D(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(out.dataId).id, convInfo
			.batchSize,
			// Since Pool3D ops (AvgPool3D and MaxPool3D) support 3D filter only, in
			// channels should always equal to out channels.
			/*channelSize=*/
	...
	function maxPool3DGrad(args) {
		...
		var convInfo = tfjsCore.backend_util.computePool3DInfo(input.shape, filterSize, strides, /*dilations=*/
			1, pad, dimRoundingMode);
		var dx = backend.makeOutput(input.shape, input.dtype);
		wasmMaxPool3DGrad(backend.dataIdMap.get(input.dataId).id, backend.dataIdMap.get(dy.dataId).id, backend
			.dataIdMap.get(dx.dataId).id, convInfo.batchSize,
	...
	function maxPoolGrad(args) {
		...
		var convInfo = tfjsCore.backend_util.computePool2DInfo(input.shape, filterSize, strides,
			...
			1, pad, dimRoundingMode);
		var dx = backend.makeOutput(input.shape, input.dtype);
		wasmMaxPoolGrad(backend.dataIdMap.get(input.dataId).id, backend.dataIdMap.get(dy.dataId).id, backend
			.dataIdMap.get(dx.dataId).id, convInfo.batchSize,
	...
	function maxPoolWithArgmax(args) {
		...
		var dilations = [1, 1];
		tfjsCore.util.assert(tfjsCore.backend_util.eitherStridesOrDilationsAreOne(strides, dilations),
			function() {
		...
		wasmMaxPoolWithArgmax(backend.dataIdMap.get(x.dataId).id, backend.dataIdMap.get(pooled.dataId).id,
			backend.dataIdMap.get(indexes.dataId).id, CppDType[x.dtype], includeBatchInIndex, convInfo
			.batchSize, convInfo.inChannels, convInfo.inHeight, convInfo.inWidth, convInfo.outHeight,
			convInfo.outWidth, convInfo.strideHeight, convInfo.strideWidth, convInfo.dilationHeight,
	...
	function mean(args) {
		...
		if (inputWasTransposed) {
			...
			if (transposedId !== xId) {
				...
				input = transposed;
				inputId = transposedId;
				reductionAxes = tfjsCore.backend_util.getInnerMostAxes(reductionAxes.length, input.shape
					.length);
			}
		}
	...
	function mirrorPad(args) {
		...
		wasmMirrorPad(xId, xShapeBytes, x.shape.length, CppDType[x.dtype], prePaddingsBytes, postPaddingsBytes,
			MirrorPaddingMode[mode], outId);
		return out;
	}
	var mirrorPadConfig = {
	...
	function multinomial(args) {
		...
		wasmMultinomial(backend.dataIdMap.get(probabilities.dataId).id, batchSize, numEvents, numSamples, seed,
	...
	function kernelFunc(args) {
		...
		var boxesId = backend.dataIdMap.get(boxes.dataId).id;
		var scoresId = backend.dataIdMap.get(scores.dataId).id;
		var resOffset = wasmFunc$1(boxesId, scoresId, maxOutputSize, iouThreshold, scoreThreshold,
			softNmsSigma);
		var _a = parseResultStruct(backend, resOffset),
			pSelectedIndices = _a.pSelectedIndices,
			selectedSize = _a.selectedSize,
	...
	function oneHot(args) {
		...
		var out = backend.makeOutput(__spreadArray(__spreadArray([], __read(indices.shape), false), [depth],
			false), dtype);
		var outId = backend.dataIdMap.get(out.dataId).id;
		var indicesData = backend.dataIdMap.get(indices.dataId);
		var indicesId = indicesData.id;
	...
	function pack(args) {
		...
		inputs.forEach(function(t) {
			tfjsCore.util.assertShapesMatch(shape, t.shape,
	...
	function pad(args) {
		...
		var prePaddingsBytes = new Uint8Array(new Int32Array(prePaddingsFlat).buffer);
		var postPaddingsBytes = new Uint8Array(new Int32Array(postPaddingsFlat).buffer);
		wasmPadV2(xId, xShapeBytes, x.shape.length, CppDType[x.dtype], prePaddingsBytes, postPaddingsBytes,
			constantValue, outId);
		return out;
	}
	var padV2Config = {
	...
	function resizeBilinear(args) {
		...
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmResizeBilinear(xId, batch, oldHeight, oldWidth, numChannels, newHeight, newWidth, alignCorners ? 1 :
			0, halfPixelCenters ? 1 : 0, outId);
		if (castedData != null) {
			backend.disposeData(castedData.dataId);
		}
		return out;
	...
	function resizeBilinearGrad(args) {
		...
		wasmResizeBilinearGrad(backend.dataIdMap.get(images.dataId).id, backend.dataIdMap.get(dy.dataId).id,
			backend.dataIdMap.get(dx.dataId).id, new Uint8Array(new Int32Array(images.shape).buffer),
			new Uint8Array(new Int32Array(dy.shape).buffer), alignCorners);
		if (castedData != null) {
			backend.disposeData(castedData.dataId);
		}
		return dx;
	...
	function resizeNearestNeighbor(args) {
		...
		var xId = xData.id;
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmResizeNearestNeighbor(xId, batch, oldHeight, oldWidth, numChannels, newHeight, newWidth,
	...
	function resizeNearestNeighborGrad(args) {
		...
		wasmResizeNearestNeighborGrad(backend.dataIdMap.get(images.dataId).id, backend.dataIdMap.get(dy.dataId)
			.id, backend.dataIdMap.get(dx.dataId).id, new Uint8Array(new Int32Array(images.shape).buffer),
			new Uint8Array(new Int32Array(dy.shape).buffer), alignCorners);
		if (castedData != null) {
			backend.disposeData(castedData.dataId);
		}
	...
	function rotateWithOffset(args) {
		...
		var fillValues = typeof fillValue === 'number' ? [fillValue, fillValue, fillValue, fillIsBlack ? 0 :
			fullOpacityValue
		...
		wasmRotate(imageId, batch, imageHeight, imageWidth, numChannels, radians, centerX, centerY, fillBytes,
			fillValues.length, outId);
		return out;
	}
	...
	function scatterNd(args) {
		...
		var stridesBytes = new Uint8Array(new Int32Array(strides).buffer);
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmScatterNd(indicesId, updatesId, CppDType[updates.dtype], sliceRank, numUpdates, sliceSize,
			stridesBytes, outputSize, outId);
		return out;
	}
	var scatterNdConfig = {
	...
	function searchSorted(args) {
		...
		if (sortedSequence.dtype !== values.dtype) {
			throw new Error("SearchSorted error: sorted_sequence must have the same dtype as values. Got "
				.concat(sortedSequence.dtype, " and ").concat(values.dtype));
		}
	...
	function spaceToBatchND(args) {
		...
		var permutedReshapedPaddedPermutation = tfjsCore.backend_util.getPermuted(reshapedPaddedShape.length,
			blockShape.length, false);
	...
	function sparseFillEmptyRows(args) {
		...
		var outputRows = wasmSparseFillEmptyRows(indicesId, valuesId, CppDType[values.dtype], indicesCount,
			denseRows, rank, defaultValueId, outputIndicesId, outputValuesId, emptyRowIndicatorId,
			reverseIndexMapId, exceptionValuesId);
		var exceptionValuesArray = backend.readSync(exceptionValues.dataId);
		var exceptionMessage;
		switch (exceptionValuesArray[0]) {
			case 1: {
				exceptionMessage =
					tfjsCore.backend_util.getSparseFillEmptyRowsIndicesDenseShapeMismatch(exceptionValuesArray[
						1]);
				break;
			}
			case 2: {
				exceptionMessage =
					tfjsCore.backend_util.getSparseFillEmptyRowsNegativeIndexErrorMessage(exceptionValuesArray[
						1], exceptionValuesArray[2]);
				break;
			}
			case 3:
				exceptionMessage =
					tfjsCore.backend_util.getSparseFillEmptyRowsOutOfRangeIndexErrorMessage(
						exceptionValuesArray[1], exceptionValuesArray[2], exceptionValuesArray[3]);
				break;
			default:
				exceptionMessage = '';
		}
	...
	function sparseReshape(args) {
		...
		var exceptionValuesId = backend.dataIdMap.get(exceptionValues.dataId).id;
		wasmSparseReshape(inputIndicesId, inputShapeId, newShapeId, nnz, newIndicesId, outputShapeId,
			exceptionValuesId);
		var exceptionValuesArray = backend.readSync(exceptionValues.dataId);
		var exceptionMessage;
		switch (exceptionValuesArray[0]) {
			case 0: {
				exceptionMessage =
					tfjsCore.backend_util.getSparseReshapeMultipleNegativeOneOutputDimErrorMessage(
						exceptionValuesArray[1], exceptionValuesArray[2]);
				break;
			}
			case 1: {
				exceptionMessage =
					tfjsCore.backend_util.getSparseReshapeNegativeOutputDimErrorMessage(exceptionValuesArray[1],
			...
			case 3: {
				...
				exceptionMessage =
					tfjsCore.backend_util.getSparseReshapeInputOutputMultipleErrorMessage(inputShapeValues,
						outputShapeValues);
				break;
			}
			case 4: {
				var inputShapeValues = Array.from(backend.readSync(inputShape.dataId)),
					outputShapeValues = Array.from(backend.readSync(outputShape.dataId));
				exceptionMessage =
					tfjsCore.backend_util.getSparseReshapeInputOutputMismatchErrorMessage(inputShapeValues,
						outputShapeValues);
				break;
			}
			default:
	...
	function sparseSegmentReduction(args, isMean) {
		...
		var exceptionValues = backend.makeOutput([4], 'int32');
		var exceptionValuesId = backend.dataIdMap.get(exceptionValues.dataId).id;
		wasmSparseSegmentReduction(dataId, CppDType[data.dtype], data.shape[0], indicesId, segmentIdsId,
		...
		switch (exceptionValuesArray[0]) {
			...
			case 2:
				exceptionMessage =
					tfjsCore.backend_util.getSparseSegmentReductionSegmentIdOutOfRangeErrorMessage(
						exceptionValuesArray[1], exceptionValuesArray[2]);
				break;
			case 3:
				exceptionMessage =
					tfjsCore.backend_util.getSparseSegmentReductionIndicesOutOfRangeErrorMessage(
						exceptionValuesArray[1], exceptionValuesArray[2], exceptionValuesArray[3]);
				break;
			default:
				exceptionMessage = '';
	...
	function sparseToDense(args) {
		...
		var stridesBytes = new Uint8Array(new Int32Array(strides).buffer);
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmSparseToDense(sparseIndicesId, sparseValuesId, sparseValues.shape.length, defaultValueId, CppDType[
			defaultValue.dtype], sliceRank, numUpdates, sliceSize, stridesBytes, outputSize, outId);
		return out;
	}
	...
	function stridedSlice(args) {
		...
		var _a = tfjsCore.slice_util.sliceInfo(x.shape, begin, end, strides, beginMask, endMask, ellipsisMask,
		...
		if (isIdentity) {
			...
			var outStridesBytes = new Uint8Array(new Int32Array(tfjsCore.util.computeStrides(finalShapeSparse))
				.buffer);
			var outId = backend.dataIdMap.get(out.dataId).id;
			wasmStridedSlice(xId, xStridesBytes, x.shape.length, beginBytes, endBytes, stridesBytes,
	...
	function stringNGrams(args) {
		...
		var _a = __read(stringNGramsImpl($data, $dataSplits, separator, nGramWidths, leftPad, rightPad,
				padWidth, preserveShortSequences), 2),
			nGrams = _a[0],
			nGramsSplits = _a[1];
	...
	function tensorScatterUpdate(args) {
		...
		var stridesBytes = new Uint8Array(new Int32Array(strides).buffer);
		var outId = backend.dataIdMap.get(out.dataId).id;
		wasmTensorScatterUpdate(indicesId, updatesId, CppDType[updates.dtype], sliceRank, numUpdates, sliceSize,
			stridesBytes, outputSize, outId, tensorId);
		return out;
	}
	...
	function transform(args) {
		...
		wasmTransform(imageId, transformsId, (transforms.shape[0] > 1), batch, outHeight, outWidth, numChannels,
			imageWidth, imageHeight, inputStrides, image.shape.length - 1, outputStrides, outShape.length -
			1, interpolationModeId, fillModeId, fillValue, outId);
		return out;
	}
	var transformConfig = {
	...
	try {
		for (var kernelConfigs_1 = __values(kernelConfigs), kernelConfigs_1_1 = kernelConfigs_1.next(); !
			kernelConfigs_1_1.done; kernelConfigs_1_1 = kernelConfigs_1.next()) {
		...
		try {
			if (kernelConfigs_1_1 && !kernelConfigs_1_1.done && (_a = kernelConfigs_1.return)) _a.call(
				kernelConfigs_1);
		} finally {
	...
	ENV.registerFlag('WASM_HAS_SIMD_SUPPORT', function() {
		return __awaiter(void 0, void 0, void 0, function() {
			return __generator(this, function(_a) {
				try {
					// This typed array passed in to TTWebAssembly.validate is WebAssembly binary
					// code. In this case it is a small program that contains SIMD
					// instructions.
					return [2 /*return*/ , TTWebAssembly.validate(new Uint8Array([
			...
			return __generator(this, function(_a) {
				...
				try {
					...
					return [2 /*return*/ , TTWebAssembly.validate(new Uint8Array([
						0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0,
						3, 2, 1, 0, 5,
						4, 1, 3, 1, 1, 10, 11, 1, 9, 0, 65, 0, 254, 16,
						2, 0, 26, 11
					]))];
				} catch (e) {
					return [2 /*return*/ , false];
	...
	var tfjsBackendWasmThreadedSimd$1 = {
		exports: {}
	};
	...
	(function(module, exports) {
		var WasmBackendModuleThreadedSimd = (function() {
			...
			return (function(WasmBackendModuleThreadedSimd) {
				...
				var Module = typeof WasmBackendModuleThreadedSimd != "undefined" ?
				...
				var ENVIRONMENT_IS_NODE = typeof process == "object" && typeof process
					.versions == "object" && typeof process.versions.node == "string";
				...
				if (ENVIRONMENT_IS_NODE) {
					...
					read_ = function(filename, binary) {
						filename = isFileURI(filename) ? new URL(filename) : nodePath
							.normalize(filename);
					...
					if (!ENVIRONMENT_IS_NODE) {
						...
						readAsync = function(url, onload, onerror) {
							...
							xhr.responseType = "arraybuffer";
							xhr.onload = function() {
								if (xhr.status == 200 || xhr.status == 0 && xhr
									.response) {
									onload(xhr.response);
									return;
								}
				...
				if (typeof TTWebAssembly != "object") {
				...
				function UTF8ArrayToString(heapOrArray, idx, maxBytesToRead) {
					...
					if (endPtr - idx > 16 && heapOrArray.buffer && UTF8Decoder) {
						return UTF8Decoder.decode(heapOrArray
							.buffer instanceof SharedArrayBuffer ? heapOrArray.slice(
				...
				function UTF8ToString(ptr, maxBytesToRead) {
					ptr >>>= 0;
					return ptr ? UTF8ArrayToString(GROWABLE_HEAP_U8(), ptr,
						maxBytesToRead) : "";
				}
				...
				function stringToUTF8(str, outPtr, maxBytesToWrite) {
					return stringToUTF8Array(str, GROWABLE_HEAP_U8(), outPtr,
						maxBytesToWrite);
				}
				...
				if (ENVIRONMENT_IS_PTHREAD) {
					...
					if (Module["wasmMemory"]) {
						...
						wasmMemory = new TTWebAssembly.Memory({
						...
						if (!(wasmMemory.buffer instanceof SharedArrayBuffer)) {
							err(
								"requested a shared TTWebAssembly.Memory but the returned buffer is not a SharedArrayBuffer, indicating that while the browser has SharedArrayBuffer it does not have WebAssembly threads support - you may need to set a flag"
							);
							if (ENVIRONMENT_IS_NODE) {
								err(
				...
				function getBinary(file) {
					try {
						...
						throw "both async and sync fetching of the wasm failed" + file;
					} catch (err) {
						abort(err);
					}
				}
				...
				function createWasm() {
					// 定义 WebAssembly 实例化时需要传递的参数信息
					var info = {
						"env": asmLibraryArg, // 传入环境参数 asmLibraryArg（可能包含一些库或接口）
						"wasi_snapshot_preview1": asmLibraryArg // 传入 WebAssembly System Interface (WASI) 参数
					};

					// WebAssembly 实例加载完成后调用的回调函数
					function receiveInstance(instance, module) {
						var exports = instance.exports; // 获取实例的导出内容
						Module["asm"] = exports; // 将导出的内容存储到 Module["asm"] 中
						// registerTLSInit(Module["asm"][
						// 	"_emscripten_tls_init"
						// ]); // 初始化线程局部存储（TLS）
						// wasmTable = Module["asm"]["__indirect_function_table"]; // 获取函数表
						addOnInit(Module["asm"]["__wasm_call_ctors"]); // 添加初始化函数
						wasmModule = module; // 保存模块信息

						// 处理 Web Worker 的加载，如果存在未使用的 Worker
						// if (!ENVIRONMENT_IS_PTHREAD) {
						// 	var numWorkersToLoad = PThread.unusedWorkers
						// 		.length; // 获取未使用的 Worker 数量
						// 	PThread.unusedWorkers.forEach(function(w) {
						// 		PThread.loadWasmModuleToWorker(w, function() {
						// 			if (!--
						// 				numWorkersToLoad
						// 				) // 当所有 Worker 加载完成时，移除依赖
						// 				removeRunDependency();
						// 		});
						// 	});
						// }
					...
					// 如果不在 Web Worker 环境中，增加运行时依赖
					// if (!ENVIRONMENT_IS_PTHREAD) {
					addRunDependency();
					// }

					// 直接使用 TTWebAssembly.instantiate 加载包内的 wasm 文件
					function instantiateAsync() {
						// 使用 TTWebAssembly.instantiate 加载包内的 wasm 文件
						return TTWebAssembly.instantiate('./tfjs-backend-wasm.wasm', info)
							.then(function(result) {
								// 实例化完成后调用 receiveInstance
								console.log(result)
								//receiveInstance(result.instance, result.module);
							})
							.catch(function(reason) {
								// 错误处理：如果 WebAssembly 实例化失败
								err("failed to asynchronously prepare wasm: " + reason);
								abort(reason);
							});
					}

					// 如果 Module 中提供了 instantiateWasm 方法，尝试调用该方法
					if (Module["instantiateWasm"]) {
						try {
							var exports = Module["instantiateWasm"](info, receiveInstance);
							return exports; // 如果成功，返回导出的内容
						} catch (e) {
							// 如果发生错误，输出错误信息并拒绝准备好的 promise
				...
				var PThread = {
					...
					terminateAllThreads: function() {
						var e_1, _a, e_2, _b;
						try {
							for (var _c = __values(Object.values(PThread.pthreads)),
						...
						try {
							for (var _e = __values(PThread.unusedWorkers), _f = _e
									.next(); !_f.done; _f = _e.next()) {
								var worker = _f.value;
								worker.terminate();
							}
						} catch (e_2_1) {
					...
					returnWorkerToPool: function(worker) {
						...
						PThread.runningWorkers.splice(PThread.runningWorkers
							.indexOf(worker), 1);
						worker.pthread_ptr = 0;
						if (ENVIRONMENT_IS_NODE) {
							worker.unref();
					...
					loadWasmModuleToWorker: function(worker, onFinishedLoading) {
						...
						worker.onmessage = function(e) {
							...
							if (worker.pthread_ptr)
								...
								worker.pthread_ptr;
							if (d["targetThread"] && d["targetThread"] !=
								_pthread_self()) {
								var targetWorker = PThread.pthreads[d
									.targetThread];
								if (targetWorker) {
									targetWorker.postMessage(d, d[
										"transferList"]);
								} else {
									err('Internal error! Worker sent a message "' +
										cmd + '" to target pthread ' + d[
											"targetThread"] +
							...
							if (cmd === "processProxyingQueue") {
								...
								out("Thread " + d["threadId"] + ": " + d[
									"text"]);
							} else if (cmd === "printErr") {
								err("Thread " + d["threadId"] + ": " + d[
									"text"]);
							} else if (cmd === "alert") {
								alert("Thread " + d["threadId"] + ": " + d[
									"text"]);
							} else if (d.target === "setimmediate") {
								worker.postMessage(d);
							} else if (cmd === "callHandler") {
								Module[d["handler"]].apply(Module,
									__spreadArray([], __read(d["args"]),
										false));
							} else if (cmd) {
						...
						worker.onerror = function(e) {
							...
							err(message + " " + e.filename + ":" + e.lineno +
								": " + e.message);
							throw e;
						};
						if (ENVIRONMENT_IS_NODE) {
						...
						var handlers = [];
						var knownHandlers = ["onExit", "onAbort", "print",
							"printErr"
						];
						try {
							for (var knownHandlers_1 = __values(knownHandlers),
									knownHandlers_1_1 = knownHandlers_1.next(); !
								knownHandlers_1_1.done; knownHandlers_1_1 =
								knownHandlers_1.next()) {
							...
							try {
								if (knownHandlers_1_1 && !knownHandlers_1_1.done &&
									(_a = knownHandlers_1.return)) _a.call(
									knownHandlers_1);
							} finally {
					...
					getNewWorker: function() {
						if (PThread.unusedWorkers.length == 0) {
							PThread.allocateUnusedWorker();
							PThread.loadWasmModuleToWorker(PThread.unusedWorkers[
								0]);
						}
				...
				function pthreadCreateProxied(pthread_ptr, attr, startRoutine, arg) {
					if (ENVIRONMENT_IS_PTHREAD)
						return _emscripten_proxy_to_main_thread_js(3, 1, pthread_ptr, attr,
							startRoutine, arg);
				...
				function __emscripten_notify_task_queue(targetThreadId, currThreadId,
					mainThreadId, queue) {
				...
				function _emscripten_proxy_to_main_thread_js(index, sync) {
					...
					return withStackSave(function() {
						...
						return _emscripten_run_in_main_runtime_thread_js(index,
				...
				function _emscripten_receive_on_main_thread_js(index, numCallArgs, args) {
					...
					for (var i = 0; i < numCallArgs; i++) {
						_emscripten_receive_on_main_thread_js_callArgs[i] =
							GROWABLE_HEAP_F64()[b + i >>> 0];
					}
					var isEmAsmConst = index < 0;
					var func = !isEmAsmConst ? proxiedFunctionTable[index] : ASM_CONSTS[-
						index - 1];
				...
				function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
					if (ENVIRONMENT_IS_PTHREAD)
						return _emscripten_proxy_to_main_thread_js(5, 1, fd, offset_low,
				...
				function _fd_write(fd, iov, iovcnt, pnum) {
					if (ENVIRONMENT_IS_PTHREAD)
						return _emscripten_proxy_to_main_thread_js(6, 1, fd, iov, iovcnt,
							pnum);
					var num = 0;
				...
				PThread.init();
				var proxiedFunctionTable = [null, _proc_exit, exitOnMainThread,
					pthreadCreateProxied, _fd_close, _fd_seek, _fd_write
				];
				var asmLibraryArg = {
				...
				Module["___wasm_call_ctors"] = function() {
					return (Module["___wasm_call_ctors"] = Module["asm"][
						"__wasm_call_ctors"
					]).apply(null, arguments);
				};
				Module["_init"] = function() {
					return (Module["_init"] = Module["asm"]["init"]).apply(null,
						arguments);
				};
				Module["_init_with_threads_count"] = function() {
					return (Module["_init_with_threads_count"] = Module["asm"][
						"init_with_threads_count"
					]).apply(null, arguments);
				};
				Module["_get_threads_count"] = function() {
					return (Module["_get_threads_count"] = Module["asm"][
						"get_threads_count"
					]).apply(null, arguments);
				};
				Module["_register_tensor"] = function() {
					return (Module["_register_tensor"] = Module["asm"][
						"register_tensor"
					]).apply(null, arguments);
				};
				Module["_dispose_data"] = function() {
					return (Module["_dispose_data"] = Module["asm"]["dispose_data"])
						.apply(null, arguments);
				};
				Module["_dispose"] = function() {
					return (Module["_dispose"] = Module["asm"]["dispose"]).apply(null,
						arguments);
				};
				Module["_Abs"] = function() {
					return (Module["_Abs"] = Module["asm"]["Abs"]).apply(null,
						arguments);
				};
				Module["_Acos"] = function() {
					return (Module["_Acos"] = Module["asm"]["Acos"]).apply(null,
						arguments);
				};
				Module["_Acosh"] = function() {
					return (Module["_Acosh"] = Module["asm"]["Acosh"]).apply(null,
						arguments);
				};
				Module["_Add"] = function() {
					return (Module["_Add"] = Module["asm"]["Add"]).apply(null,
						arguments);
				};
				Module["_AddN"] = function() {
					return (Module["_AddN"] = Module["asm"]["AddN"]).apply(null,
						arguments);
				};
				Module["_All"] = function() {
					return (Module["_All"] = Module["asm"]["All"]).apply(null,
						arguments);
				};
				Module["_Any"] = function() {
					return (Module["_Any"] = Module["asm"]["Any"]).apply(null,
						arguments);
				};
				Module["_ArgMax"] = function() {
					return (Module["_ArgMax"] = Module["asm"]["ArgMax"]).apply(null,
						arguments);
				};
				Module["_ArgMin"] = function() {
					return (Module["_ArgMin"] = Module["asm"]["ArgMin"]).apply(null,
						arguments);
				};
				Module["_Asin"] = function() {
					return (Module["_Asin"] = Module["asm"]["Asin"]).apply(null,
						arguments);
				};
				Module["_Asinh"] = function() {
					return (Module["_Asinh"] = Module["asm"]["Asinh"]).apply(null,
						arguments);
				};
				Module["_Atan"] = function() {
					return (Module["_Atan"] = Module["asm"]["Atan"]).apply(null,
						arguments);
				};
				Module["_Atan2"] = function() {
					return (Module["_Atan2"] = Module["asm"]["Atan2"]).apply(null,
						arguments);
				};
				Module["_Atanh"] = function() {
					return (Module["_Atanh"] = Module["asm"]["Atanh"]).apply(null,
						arguments);
				};
				Module["_AvgPool"] = function() {
					return (Module["_AvgPool"] = Module["asm"]["AvgPool"]).apply(null,
						arguments);
				};
				Module["_AvgPool3D"] = function() {
					return (Module["_AvgPool3D"] = Module["asm"]["AvgPool3D"]).apply(
						null, arguments);
				};
				Module["_AvgPool3DGrad"] = function() {
					return (Module["_AvgPool3DGrad"] = Module["asm"]["AvgPool3DGrad"])
						.apply(null, arguments);
				};
				Module["_AvgPoolGrad"] = function() {
					return (Module["_AvgPoolGrad"] = Module["asm"]["AvgPoolGrad"])
						.apply(null, arguments);
				};
				Module["_BatchMatMul"] = function() {
					return (Module["_BatchMatMul"] = Module["asm"]["BatchMatMul"])
						.apply(null, arguments);
				};
				Module["_Bincount"] = function() {
					return (Module["_Bincount"] = Module["asm"]["Bincount"]).apply(null,
						arguments);
				};
				Module["_BitwiseAnd"] = function() {
					return (Module["_BitwiseAnd"] = Module["asm"]["BitwiseAnd"]).apply(
						null, arguments);
				};
				Module["_Ceil"] = function() {
					return (Module["_Ceil"] = Module["asm"]["Ceil"]).apply(null,
						arguments);
				};
				Module["_ClipByValue"] = function() {
					return (Module["_ClipByValue"] = Module["asm"]["ClipByValue"])
						.apply(null, arguments);
				};
				Module["_Conv2D"] = function() {
					return (Module["_Conv2D"] = Module["asm"]["Conv2D"]).apply(null,
						arguments);
				};
				Module["_Conv2DBackpropInput"] = function() {
					return (Module["_Conv2DBackpropInput"] = Module["asm"][
						"Conv2DBackpropInput"
					]).apply(null, arguments);
				};
				Module["_Conv3D"] = function() {
					return (Module["_Conv3D"] = Module["asm"]["Conv3D"]).apply(null,
						arguments);
				};
				Module["_Conv3DBackpropFilterV2"] = function() {
					return (Module["_Conv3DBackpropFilterV2"] = Module["asm"][
						"Conv3DBackpropFilterV2"
					]).apply(null, arguments);
				};
				Module["_Conv3DBackpropInputV2"] = function() {
					return (Module["_Conv3DBackpropInputV2"] = Module["asm"][
						"Conv3DBackpropInputV2"
					]).apply(null, arguments);
				};
				Module["_Cos"] = function() {
					return (Module["_Cos"] = Module["asm"]["Cos"]).apply(null,
						arguments);
				};
				Module["_Cosh"] = function() {
					return (Module["_Cosh"] = Module["asm"]["Cosh"]).apply(null,
						arguments);
				};
				Module["_CropAndResize"] = function() {
					return (Module["_CropAndResize"] = Module["asm"]["CropAndResize"])
						.apply(null, arguments);
				};
				Module["_Cumprod"] = function() {
					return (Module["_Cumprod"] = Module["asm"]["Cumprod"]).apply(null,
						arguments);
				};
				Module["_Cumsum"] = function() {
					return (Module["_Cumsum"] = Module["asm"]["Cumsum"]).apply(null,
						arguments);
				};
				Module["_DenseBincount"] = function() {
					return (Module["_DenseBincount"] = Module["asm"]["DenseBincount"])
						.apply(null, arguments);
				};
				Module["_DepthToSpace"] = function() {
					return (Module["_DepthToSpace"] = Module["asm"]["DepthToSpace"])
						.apply(null, arguments);
				};
				Module["_DepthwiseConv2dNative"] = function() {
					return (Module["_DepthwiseConv2dNative"] = Module["asm"][
						"DepthwiseConv2dNative"
					]).apply(null, arguments);
				};
				Module["_Diag"] = function() {
					return (Module["_Diag"] = Module["asm"]["Diag"]).apply(null,
						arguments);
				};
				Module["_Dilation2D"] = function() {
					return (Module["_Dilation2D"] = Module["asm"]["Dilation2D"]).apply(
						null, arguments);
				};
				Module["_Dilation2DBackpropFilter"] = function() {
					return (Module["_Dilation2DBackpropFilter"] = Module["asm"][
						"Dilation2DBackpropFilter"
					]).apply(null, arguments);
				};
				Module["_Dilation2DBackpropInput"] = function() {
					return (Module["_Dilation2DBackpropInput"] = Module["asm"][
						"Dilation2DBackpropInput"
					]).apply(null, arguments);
				};
				Module["_Elu"] = function() {
					return (Module["_Elu"] = Module["asm"]["Elu"]).apply(null,
						arguments);
				};
				Module["_EluGrad"] = function() {
					return (Module["_EluGrad"] = Module["asm"]["EluGrad"]).apply(null,
						arguments);
				};
				Module["_Equal"] = function() {
					return (Module["_Equal"] = Module["asm"]["Equal"]).apply(null,
						arguments);
				};
				Module["_Erf"] = function() {
					return (Module["_Erf"] = Module["asm"]["Erf"]).apply(null,
						arguments);
				};
				Module["_Exp"] = function() {
					return (Module["_Exp"] = Module["asm"]["Exp"]).apply(null,
						arguments);
				};
				Module["_Expm1"] = function() {
					return (Module["_Expm1"] = Module["asm"]["Expm1"]).apply(null,
						arguments);
				};
				Module["_FlipLeftRight"] = function() {
					return (Module["_FlipLeftRight"] = Module["asm"]["FlipLeftRight"])
						.apply(null, arguments);
				};
				Module["_Floor"] = function() {
					return (Module["_Floor"] = Module["asm"]["Floor"]).apply(null,
						arguments);
				};
				Module["_FloorDiv"] = function() {
					return (Module["_FloorDiv"] = Module["asm"]["FloorDiv"]).apply(null,
						arguments);
				};
				Module["_FusedBatchNorm"] = function() {
					return (Module["_FusedBatchNorm"] = Module["asm"]["FusedBatchNorm"])
						.apply(null, arguments);
				};
				Module["_FusedConv2D"] = function() {
					return (Module["_FusedConv2D"] = Module["asm"]["FusedConv2D"])
						.apply(null, arguments);
				};
				Module["_FusedDepthwiseConv2D"] = function() {
					return (Module["_FusedDepthwiseConv2D"] = Module["asm"][
						"FusedDepthwiseConv2D"
					]).apply(null, arguments);
				};
				Module["_Gather"] = function() {
					return (Module["_Gather"] = Module["asm"]["Gather"]).apply(null,
						arguments);
				};
				Module["_GatherNd"] = function() {
					return (Module["_GatherNd"] = Module["asm"]["GatherNd"]).apply(null,
						arguments);
				};
				Module["_Greater"] = function() {
					return (Module["_Greater"] = Module["asm"]["Greater"]).apply(null,
						arguments);
				};
				Module["_GreaterEqual"] = function() {
					return (Module["_GreaterEqual"] = Module["asm"]["GreaterEqual"])
						.apply(null, arguments);
				};
				Module["_IsFinite"] = function() {
					return (Module["_IsFinite"] = Module["asm"]["IsFinite"]).apply(null,
						arguments);
				};
				Module["_IsInf"] = function() {
					return (Module["_IsInf"] = Module["asm"]["IsInf"]).apply(null,
						arguments);
				};
				Module["_IsNan"] = function() {
					return (Module["_IsNan"] = Module["asm"]["IsNan"]).apply(null,
						arguments);
				};
				Module["_LRN"] = function() {
					return (Module["_LRN"] = Module["asm"]["LRN"]).apply(null,
						arguments);
				};
				Module["_LRNGrad"] = function() {
					return (Module["_LRNGrad"] = Module["asm"]["LRNGrad"]).apply(null,
						arguments);
				};
				Module["_LeakyRelu"] = function() {
					return (Module["_LeakyRelu"] = Module["asm"]["LeakyRelu"]).apply(
						null, arguments);
				};
				Module["_Less"] = function() {
					return (Module["_Less"] = Module["asm"]["Less"]).apply(null,
						arguments);
				};
				Module["_LessEqual"] = function() {
					return (Module["_LessEqual"] = Module["asm"]["LessEqual"]).apply(
						null, arguments);
				};
				Module["_LinSpace"] = function() {
					return (Module["_LinSpace"] = Module["asm"]["LinSpace"]).apply(null,
						arguments);
				};
				Module["_Log"] = function() {
					return (Module["_Log"] = Module["asm"]["Log"]).apply(null,
						arguments);
				};
				Module["_Log1p"] = function() {
					return (Module["_Log1p"] = Module["asm"]["Log1p"]).apply(null,
						arguments);
				};
				Module["_LogicalAnd"] = function() {
					return (Module["_LogicalAnd"] = Module["asm"]["LogicalAnd"]).apply(
						null, arguments);
				};
				Module["_LogicalNot"] = function() {
					return (Module["_LogicalNot"] = Module["asm"]["LogicalNot"]).apply(
						null, arguments);
				};
				Module["_LogicalOr"] = function() {
					return (Module["_LogicalOr"] = Module["asm"]["LogicalOr"]).apply(
						null, arguments);
				};
				Module["_LogicalXor"] = function() {
					return (Module["_LogicalXor"] = Module["asm"]["LogicalXor"]).apply(
						null, arguments);
				};
				Module["_Max"] = function() {
					return (Module["_Max"] = Module["asm"]["Max"]).apply(null,
						arguments);
				};
				Module["_MaxPool"] = function() {
					return (Module["_MaxPool"] = Module["asm"]["MaxPool"]).apply(null,
						arguments);
				};
				Module["_MaxPool3D"] = function() {
					return (Module["_MaxPool3D"] = Module["asm"]["MaxPool3D"]).apply(
						null, arguments);
				};
				Module["_MaxPool3DGrad"] = function() {
					return (Module["_MaxPool3DGrad"] = Module["asm"]["MaxPool3DGrad"])
						.apply(null, arguments);
				};
				Module["_MaxPoolGrad"] = function() {
					return (Module["_MaxPoolGrad"] = Module["asm"]["MaxPoolGrad"])
						.apply(null, arguments);
				};
				Module["_MaxPoolWithArgmax"] = function() {
					return (Module["_MaxPoolWithArgmax"] = Module["asm"][
						"MaxPoolWithArgmax"
					]).apply(null, arguments);
				};
				Module["_Maximum"] = function() {
					return (Module["_Maximum"] = Module["asm"]["Maximum"]).apply(null,
						arguments);
				};
				Module["_Mean"] = function() {
					return (Module["_Mean"] = Module["asm"]["Mean"]).apply(null,
						arguments);
				};
				Module["_Min"] = function() {
					return (Module["_Min"] = Module["asm"]["Min"]).apply(null,
						arguments);
				};
				Module["_Minimum"] = function() {
					return (Module["_Minimum"] = Module["asm"]["Minimum"]).apply(null,
						arguments);
				};
				Module["_MirrorPad"] = function() {
					return (Module["_MirrorPad"] = Module["asm"]["MirrorPad"]).apply(
						null, arguments);
				};
				Module["_Mod"] = function() {
					return (Module["_Mod"] = Module["asm"]["Mod"]).apply(null,
						arguments);
				};
				Module["_Multinomial"] = function() {
					return (Module["_Multinomial"] = Module["asm"]["Multinomial"])
						.apply(null, arguments);
				};
				Module["_Multiply"] = function() {
					return (Module["_Multiply"] = Module["asm"]["Multiply"]).apply(null,
						arguments);
				};
				Module["_Neg"] = function() {
					return (Module["_Neg"] = Module["asm"]["Neg"]).apply(null,
						arguments);
				};
				Module["_NonMaxSuppressionV3"] = function() {
					return (Module["_NonMaxSuppressionV3"] = Module["asm"][
						"NonMaxSuppressionV3"
					]).apply(null, arguments);
				};
				Module["_NonMaxSuppressionV4"] = function() {
					return (Module["_NonMaxSuppressionV4"] = Module["asm"][
						"NonMaxSuppressionV4"
					]).apply(null, arguments);
				};
				Module["_NonMaxSuppressionV5"] = function() {
					return (Module["_NonMaxSuppressionV5"] = Module["asm"][
						"NonMaxSuppressionV5"
					]).apply(null, arguments);
				};
				Module["_NotEqual"] = function() {
					return (Module["_NotEqual"] = Module["asm"]["NotEqual"]).apply(null,
						arguments);
				};
				Module["_OneHot"] = function() {
					return (Module["_OneHot"] = Module["asm"]["OneHot"]).apply(null,
						arguments);
				};
				Module["_PadV2"] = function() {
					return (Module["_PadV2"] = Module["asm"]["PadV2"]).apply(null,
						arguments);
				};
				Module["_Pow"] = function() {
					return (Module["_Pow"] = Module["asm"]["Pow"]).apply(null,
						arguments);
				};
				Module["_Prelu"] = function() {
					return (Module["_Prelu"] = Module["asm"]["Prelu"]).apply(null,
						arguments);
				};
				Module["_Prod"] = function() {
					return (Module["_Prod"] = Module["asm"]["Prod"]).apply(null,
						arguments);
				};
				Module["_RealDiv"] = function() {
					return (Module["_RealDiv"] = Module["asm"]["RealDiv"]).apply(null,
						arguments);
				};
				Module["_Reciprocal"] = function() {
					return (Module["_Reciprocal"] = Module["asm"]["Reciprocal"]).apply(
						null, arguments);
				};
				Module["_Relu"] = function() {
					return (Module["_Relu"] = Module["asm"]["Relu"]).apply(null,
						arguments);
				};
				Module["_Relu6"] = function() {
					return (Module["_Relu6"] = Module["asm"]["Relu6"]).apply(null,
						arguments);
				};
				Module["_ResizeBilinear"] = function() {
					return (Module["_ResizeBilinear"] = Module["asm"]["ResizeBilinear"])
						.apply(null, arguments);
				};
				Module["_ResizeBilinearGrad"] = function() {
					return (Module["_ResizeBilinearGrad"] = Module["asm"][
						"ResizeBilinearGrad"
					]).apply(null, arguments);
				};
				Module["_ResizeNearestNeighbor"] = function() {
					return (Module["_ResizeNearestNeighbor"] = Module["asm"][
						"ResizeNearestNeighbor"
					]).apply(null, arguments);
				};
				Module["_ResizeNearestNeighborGrad"] = function() {
					return (Module["_ResizeNearestNeighborGrad"] = Module["asm"][
						"ResizeNearestNeighborGrad"
					]).apply(null, arguments);
				};
				Module["_Reverse"] = function() {
					return (Module["_Reverse"] = Module["asm"]["Reverse"]).apply(null,
						arguments);
				};
				Module["_RotateWithOffset"] = function() {
					return (Module["_RotateWithOffset"] = Module["asm"][
						"RotateWithOffset"
					]).apply(null, arguments);
				};
				Module["_Round"] = function() {
					return (Module["_Round"] = Module["asm"]["Round"]).apply(null,
						arguments);
				};
				Module["_Rsqrt"] = function() {
					return (Module["_Rsqrt"] = Module["asm"]["Rsqrt"]).apply(null,
						arguments);
				};
				Module["_ScatterNd"] = function() {
					return (Module["_ScatterNd"] = Module["asm"]["ScatterNd"]).apply(
						null, arguments);
				};
				Module["_SearchSorted"] = function() {
					return (Module["_SearchSorted"] = Module["asm"]["SearchSorted"])
						.apply(null, arguments);
				};
				Module["_SelectV2"] = function() {
					return (Module["_SelectV2"] = Module["asm"]["SelectV2"]).apply(null,
						arguments);
				};
				Module["_Selu"] = function() {
					return (Module["_Selu"] = Module["asm"]["Selu"]).apply(null,
						arguments);
				};
				Module["_Sigmoid"] = function() {
					return (Module["_Sigmoid"] = Module["asm"]["Sigmoid"]).apply(null,
						arguments);
				};
				Module["_Sign"] = function() {
					return (Module["_Sign"] = Module["asm"]["Sign"]).apply(null,
						arguments);
				};
				Module["_Sin"] = function() {
					return (Module["_Sin"] = Module["asm"]["Sin"]).apply(null,
						arguments);
				};
				Module["_Sinh"] = function() {
					return (Module["_Sinh"] = Module["asm"]["Sinh"]).apply(null,
						arguments);
				};
				Module["_Softmax"] = function() {
					return (Module["_Softmax"] = Module["asm"]["Softmax"]).apply(null,
						arguments);
				};
				Module["_Softplus"] = function() {
					return (Module["_Softplus"] = Module["asm"]["Softplus"]).apply(null,
						arguments);
				};
				Module["_SparseFillEmptyRows"] = function() {
					return (Module["_SparseFillEmptyRows"] = Module["asm"][
						"SparseFillEmptyRows"
					]).apply(null, arguments);
				};
				Module["_SparseReshape"] = function() {
					return (Module["_SparseReshape"] = Module["asm"]["SparseReshape"])
						.apply(null, arguments);
				};
				Module["_SparseSegmentReduction"] = function() {
					return (Module["_SparseSegmentReduction"] = Module["asm"][
						"SparseSegmentReduction"
					]).apply(null, arguments);
				};
				Module["_SparseToDense"] = function() {
					return (Module["_SparseToDense"] = Module["asm"]["SparseToDense"])
						.apply(null, arguments);
				};
				Module["_Sqrt"] = function() {
					return (Module["_Sqrt"] = Module["asm"]["Sqrt"]).apply(null,
						arguments);
				};
				Module["_Square"] = function() {
					return (Module["_Square"] = Module["asm"]["Square"]).apply(null,
						arguments);
				};
				Module["_SquaredDifference"] = function() {
					return (Module["_SquaredDifference"] = Module["asm"][
						"SquaredDifference"
					]).apply(null, arguments);
				};
				Module["_Step"] = function() {
					return (Module["_Step"] = Module["asm"]["Step"]).apply(null,
						arguments);
				};
				Module["_StridedSlice"] = function() {
					return (Module["_StridedSlice"] = Module["asm"]["StridedSlice"])
						.apply(null, arguments);
				};
				Module["_Sub"] = function() {
					return (Module["_Sub"] = Module["asm"]["Sub"]).apply(null,
						arguments);
				};
				Module["_Sum"] = function() {
					return (Module["_Sum"] = Module["asm"]["Sum"]).apply(null,
						arguments);
				};
				Module["_Tan"] = function() {
					return (Module["_Tan"] = Module["asm"]["Tan"]).apply(null,
						arguments);
				};
				Module["_Tanh"] = function() {
					return (Module["_Tanh"] = Module["asm"]["Tanh"]).apply(null,
						arguments);
				};
				Module["_TensorScatterUpdate"] = function() {
					return (Module["_TensorScatterUpdate"] = Module["asm"][
						"TensorScatterUpdate"
					]).apply(null, arguments);
				};
				Module["_Tile"] = function() {
					return (Module["_Tile"] = Module["asm"]["Tile"]).apply(null,
						arguments);
				};
				Module["_TopK"] = function() {
					return (Module["_TopK"] = Module["asm"]["TopK"]).apply(null,
						arguments);
				};
				Module["_Transform"] = function() {
					return (Module["_Transform"] = Module["asm"]["Transform"]).apply(
						null, arguments);
				};
				Module["_Transpose"] = function() {
					return (Module["_Transpose"] = Module["asm"]["Transpose"]).apply(
						null, arguments);
				};
				Module["__FusedMatMul"] = function() {
					return (Module["__FusedMatMul"] = Module["asm"]["_FusedMatMul"])
						.apply(null, arguments);
				};
				Module["_malloc"] = function() {
					return (Module["_malloc"] = Module["asm"]["malloc"]).apply(null,
						arguments);
				};
				Module["_free"] = function() {
					return (Module["_free"] = Module["asm"]["free"]).apply(null,
						arguments);
				};
				Module["__emscripten_tls_init"] = function() {
					return (Module["__emscripten_tls_init"] = Module["asm"][
						"_emscripten_tls_init"
					]).apply(null, arguments);
				};
				...
				Module["___errno_location"] = function() {
					return (Module["___errno_location"] = Module["asm"][
						"__errno_location"
					]).apply(null, arguments);
				};
				var __emscripten_thread_init = Module["__emscripten_thread_init"] =
					function() {
						return (__emscripten_thread_init = Module[
							"__emscripten_thread_init"] = Module["asm"][
							"_emscripten_thread_init"
						]).apply(null, arguments);
					};
				Module["__emscripten_thread_crashed"] = function() {
					return (Module["__emscripten_thread_crashed"] = Module["asm"][
				...
				Module["_emscripten_main_thread_process_queued_calls"] = function() {
					return (Module["_emscripten_main_thread_process_queued_calls"] =
						Module["asm"]["emscripten_main_thread_process_queued_calls"]
					).apply(null, arguments);
				};
				Module["_emscripten_main_browser_thread_id"] = function() {
					return (Module["_emscripten_main_browser_thread_id"] = Module["asm"]
						["emscripten_main_browser_thread_id"]).apply(null,
						arguments);
				};
				var _emscripten_run_in_main_runtime_thread_js = Module[
					"_emscripten_run_in_main_runtime_thread_js"] = function() {
					return (_emscripten_run_in_main_runtime_thread_js = Module[
						"_emscripten_run_in_main_runtime_thread_js"] = Module[
						"asm"]["emscripten_run_in_main_runtime_thread_js"]).apply(
						null, arguments);
				};
				Module["_emscripten_dispatch_to_thread_"] = function() {
					return (Module["_emscripten_dispatch_to_thread_"] = Module["asm"][
						"emscripten_dispatch_to_thread_"
					]).apply(null, arguments);
				};
				var __emscripten_proxy_execute_task_queue = Module[
					"__emscripten_proxy_execute_task_queue"] = function() {
					return (__emscripten_proxy_execute_task_queue = Module[
							"__emscripten_proxy_execute_task_queue"] = Module["asm"]
						["_emscripten_proxy_execute_task_queue"]).apply(null,
						arguments);
				};
				var __emscripten_thread_free_data = Module[
					"__emscripten_thread_free_data"] = function() {
					return (__emscripten_thread_free_data = Module[
						"__emscripten_thread_free_data"] = Module["asm"][
						"_emscripten_thread_free_data"
					]).apply(null, arguments);
				};
				var __emscripten_thread_exit = Module["__emscripten_thread_exit"] =
					function() {
						return (__emscripten_thread_exit = Module[
							"__emscripten_thread_exit"] = Module["asm"][
				...
				var _emscripten_stack_set_limits = Module["_emscripten_stack_set_limits"] =
					function() {
						return (_emscripten_stack_set_limits = Module[
							"_emscripten_stack_set_limits"] = Module["asm"][
				...
				Module["dynCall_iijjiiii"] = function() {
					return (Module["dynCall_iijjiiii"] = Module["asm"][
						"dynCall_iijjiiii"
					]).apply(null, arguments);
				};
				Module["dynCall_jiji"] = function() {
					return (Module["dynCall_jiji"] = Module["asm"]["dynCall_jiji"])
						.apply(null, arguments);
				};
					...
					listenersAdded = {
						uncaughtException: process.listeners("uncaughtException")
							.filter(function(listener) {
								return !beforeListeners.uncaughtException.indexOf(
									listener) > -1;
							}),
						unhandledRejection: process.listeners("unhandledRejection")
							.filter(function(listener) {
								return !beforeListeners.unhandledRejection.indexOf(
									listener) > -1;
							})
					};
				}
			...
			return (function(WasmBackendModule) {
				...
				function getBinary(file) {
					try {
						...
						throw "8769 both async and sync fetching of the wasm failed " +
							file;
					} catch (err) {
						abort(err);
					}
				...
				function createWasm() {
					...
					function instantiateAsync() {
						// 使用 TTWebAssembly.instantiate 加载包内的 wasm 文件
						return TTWebAssembly.instantiate('./tfjs-backend-wasm-simd.wasm', info)
							.then(function(result) {
								// 实例化完成后调用 receiveInstance
								receiveInstance(result.instance, result.module);
							})
	...
	var BackendWasm = /** @class */ (function(_super) {
		...
		BackendWasm.prototype.move = function(dataId, values, shape, dtype, refCount) {
			...
			if (values != null) {
				this.wasm.HEAPU8.set(new Uint8Array(values.buffer, values.byteOffset, numBytes),
					memoryOffset);
			}
		};
		...
		BackendWasm.prototype.readSync = function(dataId, start, end) {
			...
			var bytes = this.wasm.HEAPU8.slice(memoryOffset + start * bytesPerElement,
		...
		BackendWasm.prototype.makeOutput = function(shape, dtype, memoryOffset, values) {
			...
			if (memoryOffset == null) {
				dataId = this.write(values !== null && values !== void 0 ? values : null, shape,
					dtype);
			} else {
				var id = this.dataIdNextNumber++;
				dataId = {
		...
		return function(imports, callback) {
			tfjsCore.util.fetch(path, {
				...
				response.arrayBuffer().then(function(binary) {
					TTWebAssembly.instantiate('./tfjs-backend-wasm.wasm', imports).then(
						function(output) {
		...
		return __awaiter(this, void 0, void 0, function() {
			...
			return __generator(this, function(_b) {
				switch (_b.label) {
					...
					case 1:
						_a = __read.apply(void 0, [_b.sent(), 2]), simdSupported = _a[0],
							threadsSupported = _a[1];
						return [2 /*return*/ , new Promise(function(resolve, reject) {
							...
							factoryConfig.locateFile = function(path, prefix) {
								...
								if (path.endsWith('.wasm')) {
									return getPathToWasmBinary(simdSupported,
										threadsSupported, wasmPathPrefix !=
										null ? wasmPathPrefix : prefix);
								}
								return prefix + path;
							};
							...
							if (threadsSupported && simdSupported && wasmPath ==
								null) {
							...
							wasm.then(function(module) {
									...
									module.tfjs = {
										init: module.cwrap('init', null,
											[]),
										initWithThreadsCount: module.cwrap(
											'init_with_threads_count',
											null, ['number']),
										getThreadsCount: module.cwrap(
											'get_threads_count',
											'number', []),
										...
										disposeData: module.cwrap(
											'dispose_data',
											voidReturnType, ['number']),
										dispose: module.cwrap('dispose',
											voidReturnType, []),
									};
									resolve({
										wasm: module
